...
module.exports = {
        ...
        module.exports.getAppleMusicData(
            ...
            function (res, err) {
                if (!err) {
                    ...
                    if (
                        config.get("listenAlong") &&
                        appData.get("zephra.userId") &&
                        appData.get("zephra.userAuth")
                    ) {
                        rpc.presenceData.buttons.push({
                            label: "Listen alongᴮᴱᵀᴬ",
                            url: `https://amrpc.zephra.cloud/listen-along/${appData.get(
                                "zephra.userId"
                            )}`,
                        });
                    }
    ...
    checkAlpha: () => {
        ...
        setTimeout(() => {
            ...
            fetch(
                "https://amrpc.zephra.cloud/checkAlpha",
                {
                    cache: "no-store",
                    headers: {
                ...
                function (error, meta, body) {
                    ...
                    if (meta.status === 200) {
                        ...
                        if (!res) {
                            ...
                            dialog.showErrorBox(
                                "AMRPC Alpha",
                                langString.notification.alphaNotAllowed
                            );
                            app.exit();
                        } else
                            console.log(
                                "[API] Alpha authentication successful"
                            );
                    }
                }
            );
