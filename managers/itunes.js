const { app } = require("electron"),
    ...
    { connect, updateActivity, clearActivity, replaceRPCVars } = require("../managers/discord.js"),
    config = new Store({}),
...
iTunesEmitter.on("timeChange", async (type, currentTrack) => {
    if (!app.discord.presenceData.details || app.discord.presenceData.details.length > 128) return;
    let ct = Math.floor(Date.now() / 1000) - currentTrack.elapsedTime + currentTrack.duration;

    if (app.discord.presenceData.isLive) {
        let ctg = Math.floor(Date.now() / 1000) - app.discord.rpc.ctG.elapsedTime + app.discord.rpc.ctG.duration;
        ...
        if (difference > 99 && currentTrack.duration > 0) {
            ...
            app.discord.presenceData.endTimestamp = Math.floor(Date.now() / 1000) - currentTrack.elapsedTime + (currentTrack.duration + (config.get("performanceMode") ? 1.75 : 1));
    ...
    if (ct !== app.discord.presenceData.endTimestamp) {
        const difference = (ct > app.discord.presenceData.endTimestamp) ? ct - app.discord.presenceData.endTimestamp : app.discord.presenceData.endTimestamp - ct;
